  ...
  const getProduct = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        setIphone(
          res.data.allProduct.filter((item) => {
            return item.category === "apple";
          }).length
        );
        setOppo(
          res.data.allProduct.filter((item) => {
            return item.category === "oppo";
          }).length
        );
        setSamsung(
          res.data.allProduct.filter((item) => {
            return item.category === "samsung";
          }).length
        );
        setLg(
          res.data.allProduct.filter((item) => {
            return item.category === "lg";
          }).length
        );
        setXiaomi(
          res.data.allProduct.filter((item) => {
            return item.category === "xiaomi";
          }).length
        );
        setSony(
          res.data.allProduct.filter((item) => {
            return item.category === "sony";
          }).length
        );
        setTablet(
          res.data.allProduct.filter((item) => {
